...
function LedgerPage() {
  ...
  const [stmt, setStmt] = useState<{ account: Account; entries: any[] } | null>(
    null,
  )
  ...
  async function openStatement(a: Account) {
    setMsg("")
    try {
      setStmt(await api(`/accounts/${a.id}/statement?limit=100`))
    } catch (e: any) {
      setMsg(`⚠ ${e.message}`)
    }
  }

  // CSV download must carry the bearer token, so fetch -> Blob -> anchor (not a raw href)
  async function downloadCsv(a: Account) {
    setMsg("")
    try {
      const res = await fetch(
        `${BASE}/api/v1/ledger/accounts/${a.id}/statement.csv`,
        {
      ...
      link.href = url
      link.download = `statement-${a.name.replace(/[^a-z0-9]+/gi, "_")}.csv`
    ...
    <div className="mx-auto max-w-5xl px-6 py-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Create wallets, deposit, and transfer — every move is balanced
        ...
        <button
          ...
          onClick={() =>
            run(
              ...
              "demo data loaded — see the activity & event log",
            )
          }
        >
      ...
      {recon && (
        ...
        >
          ...
          <span className="text-muted-foreground">
            · global drift {recon.global_drift_cents} cents · the whole ledger
            nets to zero
          </span>
        </div>
      )}
        ...
        <div className="mt-5 rounded-xl border">
          <div className="flex items-center justify-between border-b bg-muted/40 px-4 py-2.5">
            <div className="text-sm font-semibold">
              Statement · {stmt.account.name}
              <span className="ml-2 font-mono text-xs text-muted-foreground">
                {dollars(stmt.account.balance_cents)}
              </span>
            </div>
            <div className="flex items-center gap-3">
              <button
                className="text-xs text-indigo-400 hover:underline"
                onClick={() => downloadCsv(stmt.account)}
              >
                Download CSV
              </button>
              <button
                className="text-xs text-muted-foreground hover:underline"
                onClick={() => setStmt(null)}
              >
                Close
              </button>
            </div>
            ...
            <tbody>
              ...
              {stmt.entries.map((e, i) => (
                <tr key={i} className="border-t">
                  ...
                  <td
                    className={`px-4 py-2 text-right font-mono tabular-nums ${e.amount_cents < 0 ? "text-red-400" : "text-emerald-400"}`}
                  >
                    {e.amount_cents < 0 ? "−" : "+"}
                    {dollars(Math.abs(e.amount_cents))}
                  </td>
                  <td className="px-4 py-2 text-right font-mono tabular-nums">
                    {dollars(e.running_balance_cents)}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              run(
                () =>
                  api("/deposit", {
                    ...
                    body: JSON.stringify({
                      to_account_id: dep.to,
                      amount_cents: Math.round(parseFloat(dep.amount) * 100),
                    }),
                  }),
                "deposited",
              )
        ...
        <Card title="Transfer" icon={Send}>
          <Select
            value={tr.from}
            onChange={(v) => setTr({ ...tr, from: v })}
            accounts={visible}
            placeholder="From"
          />
          <Select
            value={tr.to}
            onChange={(v) => setTr({ ...tr, to: v })}
            accounts={visible}
            placeholder="To"
          />
          <input
                ...
                () =>
                  api("/transfers", {
                    ...
                    body: JSON.stringify({
                      ...
                      to_account_id: tr.to,
                      amount_cents: Math.round(parseFloat(tr.amount) * 100),
                    }),
                  }),
                "transferred",
              )
        ...
        <Card title="Recurring payment" icon={Repeat}>
          <Select
            value={rec.from}
            onChange={(v) => setRec({ ...rec, from: v })}
            accounts={visible}
            placeholder="From"
          />
          <Select
            value={rec.to}
            onChange={(v) => setRec({ ...rec, to: v })}
            accounts={visible}
            placeholder="To"
          />
          <input
          ...
          <input
            ...
            value={rec.interval}
            onChange={(e) => setRec({ ...rec, interval: e.target.value })}
          />
          <button
            className={btn}
            onClick={() =>
              run(
                () =>
                  api("/recurring", {
                    method: "POST",
                    body: JSON.stringify({
                      from_account_id: rec.from,
                      to_account_id: rec.to,
                      amount_cents: Math.round(parseFloat(rec.amount) * 100),
                      interval_seconds: parseInt(rec.interval, 10),
                    }),
                  }).then(() =>
                    setRec({ from: "", to: "", amount: "", interval: "5" }),
                  ),
                ...
                {recurring.map((r) => (
                  <tr key={r.id} className="border-t">
                    <td className="px-4 py-2">
                      {nameOf(r.from_account_id)}{" "}
                      <span className="text-muted-foreground">→</span>{" "}
                      {nameOf(r.to_account_id)}
                    </td>
                    <td className="px-4 py-2 text-right font-mono tabular-nums">
                    ...
                    <td className="px-4 py-2">
                      <span
                        className={`rounded-md px-2 py-0.5 text-xs ${r.active ? "bg-emerald-500/10 text-emerald-400" : "bg-muted text-muted-foreground"}`}
                      >
...
const inp =
  "mb-2 w-full rounded-lg border bg-background px-3 py-2 text-sm outline-none focus:border-indigo-500"
const btn =
  "w-full rounded-lg bg-indigo-500 px-3 py-2 text-sm font-medium text-white transition hover:bg-indigo-400"
    ...
    >
      ...
      {accounts.map((a) => (
        <option key={a.id} value={a.id}>
          {a.name} ({dollars(a.balance_cents)})
        </option>
      ))}
    </select>
  )
